...
- Imported ingredients are stored as parsed free-text (qty/unit/name) but **not linked** to
  canonical pantry items — linking happens as your canonical catalog grows (or manually).
  This means pantry-coverage shows "unverified" for most imported recipes until then.
