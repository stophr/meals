...
export interface ExtractedPantryItem {
  name: string; // buyable ingredient name
  quantity: number;
...
const PROMPT =
  'List the food / grocery items on hand. For EACH item return: ' +
  '"name" (the plain buyable ingredient, no brand or prep words), ' +
  '"quantity" (a number), and ' +
  '"unit" (one of: lb, oz, g, kg, cup, tbsp, tsp, fl oz, ml, l, each, pack, can, bottle, bunch). ' +
  'Use whole counts with "each" when there is no weight/volume (e.g. 3 eggs -> 3 each). ' +
  'If a quantity is unclear, use 1 each. Ignore non-food objects. ' +
  'Respond ONLY with JSON {"items":[{"name","quantity","unit"}]}.';
