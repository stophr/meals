...
// Resolve a raw ingredient/product name to a canonical item via the GLOBAL alias index, falling
// back to the deterministic root name, and only creating a new item when nothing matches. This
// keeps "Pinch Of Sugar" / "Caster Sugar" from spawning fresh variants — they resolve to the
// existing "Sugar" and get recorded as aliases. Canonical items + aliases are shared by all orgs.
...
export async function resolveCanonicalItem(
  ...
  // 1. Direct alias hit.
  const alias = await prisma.ingredientAlias.findUnique({ where: { rawName: raw } });
  ...
  const rootAlias =
    rootKey === raw ? null : await prisma.ingredientAlias.findUnique({ where: { rawName: rootKey } });
  const existing = rootAlias
    ? { id: rootAlias.canonicalItemId }
    : await prisma.canonicalItem.findFirst({
        where: { name: { equals: root, mode: 'insensitive' } },
        select: { id: true },
      });
  ...
  if (existing) {
    await prisma.ingredientAlias.upsert({
      where: { rawName: raw },
      create: { rawName: raw, canonicalItemId: existing.id },
  ...
  for (const rn of new Set([raw, rootKey])) {
    await prisma.ingredientAlias.upsert({
      where: { rawName: rn },
      create: { rawName: rn, canonicalItemId: created.id },
